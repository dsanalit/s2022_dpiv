...
for j = 1:length(listOfFileNames)
    ...
    saturatedChar = char(sourceSpectraNode.getElementsByTagName('rawSaturated').item(0).getTextContent);
    ...
    integrationTimeInMicroseconds = str2double(sourceSpectraNode.getElementsByTagName('rawIntegrationTime').item(0).getTextContent);
    integrationTimeInMilliseconds = integrationTimeInMicroseconds / 1000;
    % Read the data.
    pixelValuesNode = sourceSpectraNode.getElementsByTagName('rawProcessedPixelValues').item(0);
    doubleNodeList = pixelValuesNode.getElementsByTagName('double');
    spectrumNumberOfPixels = doubleNodeList.getLength;
    metadataNumberOfPixels = str2double(sourceSpectraNode.getElementsByTagName('spectrometerNumberOfPixels').item(0).getTextContent);
    ...
    acquisitionTimeNode = sourceSpectraNode.getElementsByTagName('rawAcquisitionTime').item(0);
    ...
    for d = 1:darkSpectrumNodeList.getLength
        ...
        if ~darkSpectrumNode.hasAttribute('reference') % If it's not a reference...
            ...
            pixelValuesNode = darkSpectrumNode.getElementsByTagName('rawProcessedPixelValues').item(0);
            doubleNodeList = pixelValuesNode.getElementsByTagName('double');
            if doubleNodeList.getLength ~= numberOfPixels
    ...
    for r = 1:referenceSpectrumNodeList.getLength
        ...
        if ~ referenceSpectrumNode.hasAttribute('reference') % If it's not a reference to a reference spectrum.
            % Read the data.
            pixelValuesNode = referenceSpectrumNode.getElementsByTagName('rawProcessedPixelValues').item(0);
            doubleNodeList = pixelValuesNode.getElementsByTagName('double');
            if doubleNodeList.getLength ~= numberOfPixels
